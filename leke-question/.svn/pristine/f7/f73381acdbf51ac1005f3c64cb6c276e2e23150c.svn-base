define(function(require, exports, module) {
	...
	function Query(options) {
		...
		self.feedbackScope = ko.observable();//分享范围
	...
	Query.prototype.toJS = function() {
		var self = this;
		var result = {};
		result.feedbackScope = self.feedbackScope.peek();
	...
	function SearchVM(params, element) {
		...
		self.feedbackScopes = ko.observableArray([{name: '学校',value: 2},{name: '乐课',value: 6}]);
		self.curPage = ko.observable(1);

		self.disposables = [];

		self.disposables.push(ko.computed(function() {
			var val = self.query.feedbackScope();
	...
	SearchVM.prototype.load = _.throttle(function() {
		...
		var req = self._lastAjax = $.ajax({
			type: 'post',
			url: Leke.ctx + '/auth/common/question/feedback/details.htm',
			data: query,
			dataType: 'html'
		});
	...
	ko.components.register('feedback-list', {
