...
var listVM = {
		init: function(){
			...
			self.query.toJS = function(){
				var q = this;
				var result = {};
				['content','schoolStageId','subjectId','materialNodeId','knowledgeId','leagueId','diffLevel','questionTypeId','orderBy'].forEach(function(n){
					result[n] = q[n];
				});
				return result;
			};
		...
		bindEventQuery: function(){
			...
			$('.j-cur-league').on('click',function(event){
			...
			$('.j-league-ul').on('click','.j-league-li',function(){
				var $this = $(this);
				var leagueName = $this.data('leagueName');
				var leagueId = $this.data('leagueId');
				$('.j-league-ul').hide();
				$('.j-cur-league').html(leagueName);
				self.query.leagueId = leagueId;
				self.doSearch();
				...
				req.then(function(datas){
					...
					Leke.repo.pOpenQueFilter({
				        ...
				        onSelect: function(data) {
				        	...
				        	if(data.diffLevel){
				        		self.query.diffLevel = data.diffLevel;
				        		self.query.levelName = data.levelName;
				        	}else{
				        		self.query.diffLevel = null;
				        		self.query.levelName = '';
				        	}
		...
		load: function(){
			...
			$(".j-question-content").page({
			    url : Leke.ctx + '/auth/teacher/question/leaguePad/details.htm',
