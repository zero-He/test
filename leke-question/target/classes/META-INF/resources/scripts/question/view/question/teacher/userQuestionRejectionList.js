define(function(require, exports, module) {
	...
	var QuestionFeedbackList = {
			...
			fIgnore : function(evt) {
				var self = this;
				dialog.confirm("确定要忽略吗？").done(function(sure){
					if(sure) {
						var $el = $(evt.target);
						var nQuestionId = $el.data('id');
						$.ajax({
							type : 'post',
							url : ctx + '/auth/teacher/questionRejection/ignore.htm',
							data : {
							...
							success: function(json) {
								...
								if(json && json.success) {
									utils.Notice.alert('忽略成功！');
									self.fLoadList();
								} else {
									utils.Notice.alert(msg || '忽略失败');
								}
							}
						});
					}
			...
			fLoadList : function() {
				var request = $.ajax({
					type : 'post',
					url : window.ctx + '/auth/teacher/questionRejection/userQuestionRejectionListDetails.htm',
					data : {},
					dataType: 'html'
				});
