...
<c:if test="${not empty records}">
	<table class="c-exercisestable">
		<tbody>
			...
			<tr class="item" data-question-id="${record.resId}">
				<td><span class="checkbox"> <input type="checkbox" name="resId" value="${record.resId}" data-question-type-id="${record.questionTypeId}"></span></td>
				<td><i class="c-restype question"></i>
					<div class="item maxwidthexercises">
						...
						<div class="remarks">
	             			<span>状态：<c:out value="${record.attrs.assoc.statusStr}"></c:out></span>
	             			<span class="source" title="${record.attrs.assoc.editLog}"><c:out value="${record.attrs.assoc.editLog}" ></c:out></span>
