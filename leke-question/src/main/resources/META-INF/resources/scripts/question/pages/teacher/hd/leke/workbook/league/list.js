...
listPage.prototype.bind=function(){
    	...
    	//$('#Jpage').unbind();
    	$('#Jpage').page({
            url : '/auth/teacher/hd/leke/workbook/league/details.htm',
            form: $("#Jform").get(0),
...
listPage.prototype.initial=function(){
    var _this =this;
   	$(".sort ").click(function(){
    	...
    	$(".resouceselect  .c-droplist li").live("click", function(){
	    ...
	    $.post("/auth/global/beike/common/ajax/microcourse/getLeague.htm",{},function(data){
				var ul = $(".resouceselect ul");
				ul.html("");
				for(var index in data.datas.leagues){
		
					var league = data.datas.leagues[index];
					if(index==0){
						$(".myres").html(league.leagueName);
						$("input[name='leagueId']").val(league.leagueId);
					}
					var li = $('<li class="item myleague" id='+league.leagueId+'>'+league.leagueName+'</li>');
					ul.append(li);
					li.click(function(){
						//$(this).addClass("cur")
			    		$("input[name='leagueId']").val($(this).attr("id"));
			    		_this.bind();
			    		$("#type").hide();
		    		});		
			 }
			 
			 $(".myleague").first().addClass("cur");
		     _this.SetUserPref();
		});
