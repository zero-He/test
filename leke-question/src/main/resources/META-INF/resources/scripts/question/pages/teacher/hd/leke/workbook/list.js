...
listPage.prototype.init=function(){
	...
	$(".sx").live("click",function(){
	 ...
	 $.post("/auth/teacher/courseware/getLeague.htm",{},function(data){
				...
				for(var index in data.datas.leagues){
					...
					var li = $('<li class="item" id='+league.leagueId+'>'+league.leagueName+'</li>');
					ul.append(li);
					li.click(function(){
						//$(this).addClass("cur")
			    		$("input[name='leagueId']").val($(this).attr("id"));
...
listPage.prototype.bind=function(){
    	...
    	//$('#Jpage').unbind();
    	$('#Jpage').page({
            url : '/auth/teacher/hd/leke/workbook/details.htm',
            form: $("#Jform").get(0),
