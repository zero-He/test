...
public class AnswerUtils {
	...
	// 判定批改是否正确
	public static final Predicate<QuestionResult> decideIsRight = v -> Boolean.TRUE.equals(v.getTotalIsRight());
	// 判定是否订正通过
	public static final Predicate<QuestionResult> decideIsPassedFix = v -> Boolean.TRUE.equals(v.getTotalIsRight())
	 ...
	 * @param oldQuestionResultList
	 */
	public static void mergeOldIsRight(Map<Long, QuestionResult> questionResultMap, List<QuestionResult> oldQuestionResultList) {
		oldQuestionResultList.forEach(oldQuestionResult -> {
			if (decideIsRight.test(oldQuestionResult)) {
				// 正确的题目直接用旧数据替换新数据
				questionResultMap.put(oldQuestionResult.getQuestionId(), oldQuestionResult);
				return;
			}
			if (oldQuestionResult.getSubs() == null) {
				return;
			}
			QuestionResult newQuestionResult = questionResultMap.get(oldQuestionResult.getQuestionId());
			if (newQuestionResult != null) {
				if (newQuestionResult.getSubs() == null) {
					newQuestionResult.setSubs(new ArrayList<>());
				}
				mergeSubsOldIsRight(newQuestionResult.getSubs(), oldQuestionResult.getSubs());
			}
		});
	}
	
	private static void mergeSubsOldIsRight(List<QuestionResult> newSubs, List<QuestionResult> oldSubs) {
		List<QuestionResult> oldRights = oldSubs.stream().filter(v -> decideIsRight.test(v)).collect(toList());
		Set<Long> oldRightIds = oldRights.stream().map(QuestionResult::getQuestionId).collect(toSet());
		newSubs.stream().filter(v -> !oldRightIds.contains(v.getQuestionId())).collect(toList());
		oldRights.forEach(oldQuestionResult -> {
			for (int i = 0; i < newSubs.size(); i++) {
				QuestionResult newQuestionResult = newSubs.get(i);
				if (oldQuestionResult.getQuestionId().equals(newQuestionResult.getQuestionId())) {
					newSubs.set(i, oldQuestionResult);
					return;
				}
			}
			newSubs.add(oldQuestionResult);
		});
	}
	 ...
	 * @param questionResultMap
	 */
	public static void fillQuestionResultNoCorrect(Map<Long, QuestionResult> questionResultMap) {
		questionResultMap.forEach((questionId, questionResult) -> {
			if (CollectionUtils.isNotEmpty(questionResult.getSubs())) {
				questionResult.getSubs().stream().filter(v -> v.getTotalIsRight() == null)
						.forEach(v -> fillQuestionResultIsWrong(v));
			}
			if (questionResult.getTotalIsRight() == null) {
				fillQuestionResultIsWrong(questionResult);
			}
		});
	}
	
	private static void fillQuestionResultIsWrong(QuestionResult questionResult) {
	...
	public static void fillNullQuestionResult(Map<Long, QuestionResult> questionResultMap, PaperVisitor paper) {
		for (QuestionGroup questionGroup : paper.getPaperDetail().getGroups()) {
			for (ScoredQuestion scoredQuestion : questionGroup.getQuestions()) {
				...
				if (questionResult != null && scoredQuestion.getHasSub()) {
					Map<Long, QuestionResult> questionResultMap2 = questionResult.getSubs().stream()
							.collect(toMap(QuestionResult::getQuestionId, v -> v));
					List<QuestionResult> subs = new ArrayList<>();
					for (ScoredQuestion childScoredQuestion : scoredQuestion.getSubs()) {
						QuestionResult questionResult2 = questionResultMap2.get(childScoredQuestion.getQuestionId());
						if (questionResult2 == null) {
							questionResult2 = new QuestionResult();
							questionResult2.setQuestionId(childScoredQuestion.getQuestionId());
							questionResultMap2.put(childScoredQuestion.getQuestionId(), questionResult2);
						}
						QuestionDTO questionDTO = paper.getQuestion(childScoredQuestion.getQuestionId());
						QuestionCheckAdapter.check(questionDTO, questionResult2, childScoredQuestion);
						questionResult.getSubs().add(questionResult2);
						subs.add(questionResult2);
					}
					questionResult.setTotalResultScore(new BigDecimal(0));
					questionResult.setTotalScoreRate(new BigDecimal(0));
					questionResult.setTotalIsRight(false);
	...
	private static void mergeQuestionResultProps(QuestionResult questionResult1, QuestionResult questionResult2) {
		if (QuestionTagUtils.isSheetAnswer(questionResult1.getAnswerResults())) {
			...
			for (int i = 0; i < questionResult1.getAnswerResults().size(); i++) {
				...
				if (i < questionResult2.getAnswerResults().size()) {
					...
					// 溢出
					answerResult1.setResultScore(new BigDecimal(0));
					answerResult1.setIsRight(true);
				}
			}
		}
