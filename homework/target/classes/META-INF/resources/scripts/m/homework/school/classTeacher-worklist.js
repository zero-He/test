...
function initData() {
	...
	$('.main-list').on('click', '.item', function () {
		...
		var classId = $('.main-list-class').find('.c-btn-green').data('classid');
		var subjectId = $('.main-list-subject').find('.c-btn-green').data('subjectid');
		var jclassId = $("#jClassId").val();
		var jsubjectId = $("#jSubjectId").val();
		if (classId === undefined) {
			if (jclassId != null && jclassId != '') {
				classId = jclassId;
			} else {
				classId = null;
			}
 ...
 * 初始化呼出层中的筛选列表
 */
function initClassSubjectDataList() {
	$.ajax({
		"url": "filters.htm",
		"type": "POST",
		"dataType": "JSON",
		success: function (result) {
			...
			if (res.success) {
				//班级列表
				var classHtmlTmpl = $("#myClassContext_tpl").html();
				var classTemplate = Handlebars.compile(classHtmlTmpl);
				var classResultData = res.datas;
				var classContent = classTemplate(classResultData);
				$(".class-context").html(classContent);
				//学科列表
