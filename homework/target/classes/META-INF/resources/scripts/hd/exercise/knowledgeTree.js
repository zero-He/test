	...
	var knowledgeSetting = {
		...
		callback : {
			...
			onClick: function (e, treeId, treeNode, clickFlag) {
				$.fn.zTree.getZTreeObj("knowledgeTree").checkNode(treeNode, !treeNode.checked, true); 
	...
	var Exercise = {
		...
		binEvent : function() {
			...
			/*	if(level ==''){
				...
				//window.auto_practice.start(exerciseType, exerciseName.join(","),JSON.stringify(relId), schoolStageId, schoolStageName, subjectId, subjectName,level);
				LeKeBridge.sendMessage2Native('start', JSON.stringify({
					'exerciseType' : exerciseType,
					'exerciseName' : exerciseName.join(","),
