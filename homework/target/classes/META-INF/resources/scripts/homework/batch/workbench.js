define(function(require, exports, module) {
	...
	var Tools = require("../common/sheet.tools");
	...
	var Workbench = {
		...
		backList : function(){
			...
			setTimeout(function(){
				that.state.isExam == true ? (window.location.href = '/auth/teacher/exam/batchByQuestions.htm?homeworkId=' + that.state.homeworkId) : (window.location.href = 'batchQuestions.htm?homeworkId=' + that.state.homeworkId);
			},2000);
		},
		/*
		 * 1:代表操作 第一个 按钮
		...
		handleSaveCorrect : function(btnName,isNext) {
			...
			var questionResult = Correct.fParseQuestion($('#jQuestionBody'));
			var dfd = $.Deferred();
			if(questionResult.hasNull && !isNext){
				return dfd.resolve();
			}
			return this.doConfirm(questionResult.hasNull, message).then(function() {
				$('#j_firstStu,#j_prevStu,#j_nextStu').lock();
				var questionResultJson = JSON.stringify(questionResult.questionResultList[0]);
				$.post('save.htm', {
					homeworkDtlId : that.state.homeworkDtlId,
					questionResultJson : questionResultJson
				}, function(json) {
					if (json.success) {
						dfd.resolve();
					} else {
		...
		setGroupTitle : function(){
			var _this = this;
			var index = ' 第' + $('.j-question').data('ord') + '题';
			$.post('/auth/teacher/batch/groupTitle.htm',{paperId:_this.state.paperId,questionId:_this.state.questionId},function(json){
				var text = json.datas.groupTitle + index;
				$('#jIndex').text(text) ;
				$('#jIndex').attr('title',text);
			})
		},
		...
		doPostSubmitHwDtls : function(){
			var dfd = $.Deferred();
			var _this = this;
			$.post('/auth/teacher/batch/submitHwDtls.htm',{paperId:_this.state.paperId,homeworkId:_this.state.homeworkId,questionId:_this.state.questionId,cutDate:_this.state.cutDate},function(json){
