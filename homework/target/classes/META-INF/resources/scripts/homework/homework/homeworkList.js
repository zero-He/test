			...
			this.page = Page.create({
				...
				callback : function(datas) {
					...
					$.each(page.dataList, function() {
						...
						self.isInvalid = self.status == 2 ? true : [];
						self.isInclass = self.usePhase && self.usePhase > 0;
						self.isBeikeRes = self.courseSingleId && self.courseSingleId != 0;
					});
			...
			$(document).on('click','.j-btn-cuijiao',function(){
				...
				$.post('cuijiao.htm',{homeworkId:hwId},function(json){
					...
					Dialog.confirm(confirm_msg).done(function(sure) {
						if (sure) {
							$.post('doCuijiao.htm',{'homeworkId':hwId},function(json){
								if(json.success){
									Notice.alert(json.message);
								}
							});
						}
					});
