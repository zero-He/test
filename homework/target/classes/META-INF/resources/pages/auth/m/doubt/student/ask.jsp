...
<body style="padding-top:0px;" onload="onLoad();">
    <%-- <header class="c-header">
        <a class="back" href="<c:if test="${backStr!=null}">${backStr}</c:if><c:if test="${backStr==null}">list.htm?resolved=${resolved}</c:if>"></a>
        	我要提问
        <span id="jSubmit" class="operation">提交</span>
    </header> --%>
    <input id="jBackUrl" type="hidden" value="<c:if test="${backStr!=null}">${backStr}</c:if><c:if test="${backStr==null}">list.htm?resolved=${resolved}</c:if>">
    ...
    <section class="c-question-edit" style="top:0px">
        ...
        <div class="choose c-arrow" id="j-choose">
            	问题所属科目和老师 <span class="result" data-subjectid="${doubt.subjectId}" data-teacherid="${teacherId}">未选择</span>
        </div>
        ...
        <!-- 选择教材版本 -->
        <div class="choose-version c-hidden j-choose-version" data-v="m" data-i="${schoolStageId}">
    ...
    <script src="/scripts/m/doubt/ask.js?_tt=20171115"></script>
</body>
</html>