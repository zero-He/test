...
public class HomeworkStatusExportController {
	...
	@ResponseBody
	public JsonResult detail(ClassHomeworkQuery query ) {
		Long schoolId = UserContext.user.getCurrentSchoolId();
		query.setSchoolId(schoolId);
		List<Map<String, Object>> export = homeworkStatusExportService.getStatusData(query, false);
		List<Homework> homeWorkList = homeworkStatusExportService.getHomeWork(query);
	...
	public void export(ClassHomeworkQuery query, HttpServletRequest request, HttpServletResponse response) throws IOException {
		Long schoolId = UserContext.user.getCurrentSchoolId();
		query.setSchoolId(schoolId);
		List<Map<String, Object>> export = homeworkStatusExportService.getStatusData(query, true);
		if(export==null){
