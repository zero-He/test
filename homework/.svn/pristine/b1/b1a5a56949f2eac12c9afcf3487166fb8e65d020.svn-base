...
var RepoSelect = require('repository/select/p-repo-select');
var RepoHistory = require('repository/service/HistoryService');
...
function openRepoChoiceWin(resType) {
  ...
  RepoHistory.loadHistory().done((datas) => {
    if (resType == 1) {
      RepoSelect.openCouSelect({
        datas: datas,
        onSelect: callback
      });
    } else if (resType == 2) {
      RepoSelect.openMicSelect({
        datas: datas,
        onSelect: callback
      });
    } else {
