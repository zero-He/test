...
public class HomeworkServiceImpl implements HomeworkService {
	...
	@Override
	public List<Homework> findLessonWorkByLessonId(Long teacherId, Long lessonId, Integer resType) {
		return this.homeworkDao.findLessonWorkByLessonId(teacherId, lessonId, resType);
	}
	...
	@Override
	public List<LessonResInfo> findLessonResInfos(Long studentId, List<Long> lessonIds, Integer usePhase) {
		List<LessonResInfo> list = new ArrayList<LessonResInfo>();
		lessonIds.forEach(lessonId -> {
			LessonResInfo item = new LessonResInfo();
			item.setLessonId(lessonId);
			if(HomeworkCst.HOMEWORK_USE_PHASE_BEFORE == usePhase) {
				List<Long> hwIds = homeworkDao.findPreHwIdsByLesson(lessonId);
				if (CollectionUtils.isNotEmpty(hwIds)) {
					Map<String, Long> map = homeworkDtlDao.getHwResFinishInfo(studentId, hwIds);
					item.setTotal(map.get("total").intValue());
					item.setFinishTotal(map.get("finishTotal").intValue());
				}
			} else {
				//复习
				item.setTotal(homeworkDtlDao.getReviewHwResTotal(lessonId).intValue());
			}
			list.add(item);
		});
		return list;
	...
	@Override
	public List<Homework> findAfterHwByLessonId(Long teacherId, Long lessonId) {
		return this.homeworkDao.findAfterHwByLessonId(teacherId, lessonId);
	}
}
