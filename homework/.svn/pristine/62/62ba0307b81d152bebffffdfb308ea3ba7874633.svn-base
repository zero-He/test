...
class UserNameMatchCombo extends Component {
    ...
    $(this.refs.ipt).autocomplete({
      ...
      textFn: function(item){
        if (item) {
          ...
          let stuno = item.stuno ? `(${item.stuno})` : '';
          return `${item.lekeNo}${stuno} ${item.userName} ${item.className}${submitFlag}`;
        }
        return '';
      },
  ...
  render() {
    ...
    return <input ref="ipt" type="text" style={{width:282}} disabled={disabled} placeholder="请输入姓名/乐号/学号" />;
  }
}
