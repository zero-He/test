...
public class HomeworkSnapshotServiceImpl implements HomeworkSnapshotService {
	...
	public void saveFlowAfterClassByLessonId(Long lessonId) {
		...
		//最后一个阶段是课中作业，处理学生作业未待订正
		List<Homework> homeworks = homeworkDao.findHomeworkByCourseSingleId(lessonId);
		homeworks.stream().filter(v->v.getResType().equals(ResType.PAPER.value) && PhaseUtils.isLastPhase(v.getUsePhase(), v.getComboPhase())).forEach(v->{
			List<HomeworkDtl> hwDtls = homeworkDtlDao.findHomeworkDtlListByHomeworkId(v.getHomeworkId());
			hwDtls.stream().filter(dtl->dtl.getCorrectTime() != null && !dtl.getScoreRate().equals(BigDecimal.ONE)).forEach(dtl->{
				dtl.setBugFixStage(HomeworkCst.HOMEWORK_BUGFIX_STAGE_BUGFIX);
				homeworkDtlDao.updateHomeworkDtl(dtl);
			});
		});
	...
	public Homework saveSnapshotAndNextPhase(Long homeworkId, Integer nextPhase, Date startTime, Date closeTime,
		...
		if(homework.getComboPhase() != null && !PhaseUtils.validatePhase(homework.getComboPhase(),PhaseUtils.parsePhase(nextPhase))) {
			return null;
		}
		// 作业修改为镜像
