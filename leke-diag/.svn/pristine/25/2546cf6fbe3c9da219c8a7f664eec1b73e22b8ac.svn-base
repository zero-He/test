	...
	var Statistics = {
			...
			handleBarLineData : function(dataList, chartType, compType){
				...
				for(var i = 0; i < dataList.length; i++){
					...
					attendLessonRateData.push(dataList[i].attendLessonRate);
				}
				
				if(chartType == "line"){
					seriesData.push({name:"上课率", type:chartType, data:attendLessonRateData});
					RenderEchart.renderBarLine('trendInfo', seriesData, ["上课率"], xData, chartType, '{b} <br/>{a} : {c}%', '%', ["#619eed"]);
				}else{
					seriesData.push({name:"上课率", type:chartType, data:attendLessonRateData});
					RenderEchart.renderBarLine('compInfo', seriesData, ["上课率"], xData, chartType, '{b} <br/>{a} : {c}%', '%', ["#619eed"]);
				}
			},
			
			loadDtlDataPage : function(param){
				var url = "/auth/provost/teachingMonitor/lessonAttend/findLessonAttendRateDtl.htm?startDate="+param.startDate+"&endDate="+param.endDate+
			...
			loadDtlData : function(param){
				$.post("/auth/provost/teachingMonitor/lessonAttend/findLessonAttendRateDtl.htm", param, function(json){
					var data = json.datas;
					if (data) {
			...
			loadDtlOrderData : function(param){
				$.post("/auth/provost/teachingMonitor/lessonAttend/findLessonAttendRateDtlOrder.htm", param, function(json){
					var data = json.datas;
					if (data) {
						var beikeRateDtlList = Mustache.render($("#dtlTpl").html(), data);
						$("#jtbodyData").html(beikeRateDtlList);
			...
			loadStatData : function(param){
				$.post("/auth/provost/teachingMonitor/lessonAttend/findAllLessonAttendRateStatInfo.htm", param, function(json){
					var data = json.datas;
					var stat = data.statInfo;
					var trendList = data.trendList;
					var compList = data.compList;
					if (data) {
						var pieData = RenderEchart.buildStatPieData(2, ["实上课", "未上课"], [stat.attendLesson, stat.notAttendLesson]);
						RenderEchart.renderStatInfoPie("statInfo", pieData, ['#619eed', '#ff6666']);
			...
			loadTrendData : function(param){
				$.post("/auth/provost/teachingMonitor/lessonAttend/findLessonAttendRateByTrendType.htm", param, function(json){
					var data = json.datas;
					var trendList = data.lessonAttendTrendList;
					if (data) {
						//处理日期趋势数据
						Statistics.handleBarLineData(trendList, "line", '');
					}
			...
			loadCompData : function(param){
				$.post("/auth/provost/teachingMonitor/lessonAttend/findLessonAttendRateByCompType.htm", param, function(json){
					var data = json.datas;
					var compList = data.lessonAttendRateCompList;
					if (data) {
						//处理横向对比数据
						Statistics.handleBarLineData(compList, "bar", param.compType);
					}
