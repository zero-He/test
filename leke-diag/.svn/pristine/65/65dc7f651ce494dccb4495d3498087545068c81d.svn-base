	...
	var Statistics = {
		...
		bindEvent: function () {
			...
			$("#export").on("click", function () {
				...
				var form = $("form");
				form.attr("action", "/auth/provost/teachingMonitor/resource/exportResourceDtlData.htm");
				form.submit();
			});
		},
		...
		//走势、对比数据获取
		handleBarLineData: function (dataList, chartType, compType, type) {
			var xData = [];
			var seriesData = [];
			var createData = [];
			var shareData = [];
			for (var i = 0; i < dataList.length; i++) {
				...
				} else if (compType == "collect") {
					xData.push(dataList[i].resTypeName);
				} else {
					xData.push(dataList[i].dateStr);
				}
				createData.push(dataList[i].createCount);
				shareData.push(dataList[i].shareCount);
			}

			if (type == "trendList") {
				seriesData.push({name: "创建", type: chartType, data: createData});
				seriesData.push({name: "分享", type: chartType, data: shareData});
				RenderEchart.renderBarLine('trendInfo', seriesData, ["创建", "分享"], xData, chartType, '{b} <br/>{a} : {c}<br/>{a1} : {c1}', '', ['#24bdee', '#ffcb6b']);
			} else if (type == "compList") {
				seriesData.push({name: "创建", type: chartType, data: createData});
				seriesData.push({name: "分享", type: chartType, data: shareData});
				RenderEchart.renderBarLine('compInfo', seriesData, ["创建", "分享"], xData, chartType, '{b} <br/>{a} : {c}<br/>{a1} : {c1}', '', ['#24bdee', '#ffcb6b']);
			} else if (type == "countBean") {
				seriesData.push({name: "创建", type: chartType, data: createData});
				seriesData.push({name: "分享", type: chartType, data: shareData});
				RenderEchart.renderBarLine('statInfo', seriesData, ["创建", "分享"], xData, chartType, '{b} <br/>{a} : {c}<br/>{a1} : {c1}', '', ['#24bdee', '#ffcb6b']);
			}
		},

		//首次数据渲染
		loadStatData: function (param) {
			$.post("/auth/provost/teachingMonitor/resource/queryResourcePageData.htm", param, function (json) {
				...
				if (data) {
					//统计
					Statistics.handleBarLineData(countBean, "bar", 'collect', 'countBean');
					//走势
					Statistics.handleBarLineData(trendList, "line", '', 'trendList');
					//对比
					Statistics.handleBarLineData(compList, "bar", param.compType === 'clazz' ? 'grade_subject' : param.compType, 'compList');

					Handlebars.render("#topFiveTplByCreate", data, "#topFiveBodyByCreate");
					Handlebars.render("#lastFiveTplByCreate", data, "#lastFiveBodyByCreate");
					Handlebars.render("#topFiveTplByShare", data, "#topFiveBodyByShare");
					Handlebars.render("#lastFiveTplByShare", data, "#lastFiveBodyByShare");
		...
		loadDtlDataPage: function (param) {
			var url = "/auth/provost/teachingMonitor/resource/queryResourceDetailDataPage.htm?startDate=" + param.startDate + "&endDate=" + param.endDate +
