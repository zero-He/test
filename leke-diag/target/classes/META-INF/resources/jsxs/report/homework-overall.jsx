		...
		let rows = queGroups.map(item => {
			return (
				<tr key={item.id}>
					...
					<td>{toFixed(item.classScore, 1)}</td>
					...
					<table>
						<thead>
							<tr>
								...
								<th style={{width: '15%'}}>总题数</th>
								<th style={{width: '15%'}}>总分数</th>
								<th style={{width: '15%'}}>平均得分</th>
	...
	render() {
		...
		return (
			<div className="knowledgepoint">
				<span className="tittle"></span>
				<KnoGraspTable knoRates={knoGroups} columns={columns} increase={ReportCst.device != 'hd'} classId={classId} subjectId={subjectId} />
			</div>
		);
	}
}
