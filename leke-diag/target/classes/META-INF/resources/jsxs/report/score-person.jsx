...
class PersonSummary extends React.Component {
	renderExplain(summary, userName, myself) {
		...
		return (
			<p className="details">
				在此期间内，{myself ? '您' : `${userName}同学`}应交作业<span className="colored">{totalNum}</span>份，
				...
				其中成绩优秀<span className="colored">{levelA}</span>份，占比<span className="colored">{toRate(levelA, submitNum, 1)}%</span>，
		...
		return scoreRanks.map((scoreRank, i) => {
			...
			return (
				<tr key={scoreRank.userId}>
					...
					{subjs.map(subj => {
						return <td key={subj.id} className={cx({chosen: subj.id == subjId})}>{toFixed(scoreRank[subj.id], 1)}</td>;
					})}
				</tr>
			);
		})
...
class CourseRecommend extends React.Component {
	render() {
		...
		return (
			<div className="courserecommend">
				<span className="tittle">课程推荐</span>
				<span className="non-class"></span>
			</div>
		);
	}
...
class PersonAnalysis extends React.Component {
	...
	renderBody() {
		...
		if (view.summary.correctNum == 0) {
			return <PersonSummary summary={view.summary} userName={ReportCst.userName} myself={true} />
		} else {
			return (
				<div>
					<PersonSummary summary={view.summary} userName={ReportCst.userName} myself={true} />
					...
					{subjId > 0 ? <MicroRecommend micros={view.micros} /> : null}
					{subjId == 0 ? <CourseRecommend /> : null}
				</div>
			);
		}
	}
...
class DetailAnalysis extends React.Component {
	renderBody() {
		let {subjId, view} = ReportCst;
		if (view.success == false) {
			return <div className="m-tips"><i></i><span>{view.message}</span></div>;
		}
		if (view.summary.correctNum == 0) {
			return <PersonSummary summary={view.summary} userName={ReportCst.userName} myself={false} />
		} else {
			return (
				<div>
					<PersonSummary summary={view.summary} userName={ReportCst.userName} myself={false} />
					<ScoreTrendInfo trends={view.trends} subjId={subjId} />
