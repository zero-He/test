	...
	var CommEvent = {
			bindEvent : function(){
				...
				$("#customStart").on("click",function(){
					...
					if (nowDate > new Date(currYear + "-07-31") || nowDate < new Date(currYear + "-02-01")) {
						currYear = nowDate < new Date(currYear + "-02-01") ? currYear - 1 : currYear;
						min = currYear + "-08-01";
					} else {
						min = currYear + "-02-01";
					}
					...
					WdatePicker({
						...
						minDate : HandleCommParam.getDateByDelta(now, -365),
						readOnly : true
					});
