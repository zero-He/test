	...
	var Statistics = {
		...
		bindEvent: function () {
			...
			$("#export").on("click", function () {
				...
				var form = $("form");
				form.attr("action", "/auth/provost/teachingMonitor/attendance/exportAttendanceDtlData.htm");
				form.submit();
			});
		},
		...
		handleBarLineData: function (dataList, chartType, compType) {
			...
			for (var i = 0; i < dataList.length; i++) {
				...
				allOnData.push(dataList[i].allOn);
				lateData.push(dataList[i].late);
				earlyData.push(dataList[i].early);
				lateAndEarlyData.push(dataList[i].lateAndEarly);
				notClassNumData.push(dataList[i].notClassNum);
			}

			if (chartType == "line") {
				seriesData.push({name: "全勤", type: chartType, data: allOnData});
				seriesData.push({name: "迟到", type: chartType, data: lateData});
				seriesData.push({name: "早退", type: chartType, data: earlyData});
				seriesData.push({name: "迟到且早退", type: chartType, data: lateAndEarlyData});
				seriesData.push({name: "缺勤", type: chartType, data: notClassNumData});
				RenderEchart.renderBarLine('trendInfo', seriesData, ["全勤", "迟到", "早退", "迟到且早退", "缺勤"], xData, chartType, '{b} <br/>{a} : {c}<br/>{a1} : {c1}<br/>{a2} : {c2}<br/>{a3} : {c3}<br/>{a4} : {c4}', '', ['#1fb5ab', '#ff9900', '#619eed', '#ffcb6b', '#ff6666']);
			} else {
				seriesData.push({name: "全勤", type: chartType, stack: '1', data: allOnData});
				seriesData.push({name: "迟到", type: chartType, stack: '1', data: lateData});
				seriesData.push({name: "早退", type: chartType, stack: '1', data: earlyData});
				seriesData.push({name: "迟到且早退", type: chartType, stack: '1', data: lateAndEarlyData});
				seriesData.push({name: "缺勤", type: chartType, stack: '1', data: notClassNumData});
				RenderEchart.renderBarLine('compInfo', seriesData, ["全勤", "迟到", "早退", "迟到且早退", "缺勤"], xData, chartType, '{b} <br/>{a} : {c}<br/>{a1} : {c1}<br/>{a2} : {c2}<br/>{a3} : {c3}<br/>{a4} : {c4}', '', ['#1fb5ab', '#ff9900', '#619eed', '#ffcb6b', '#ff6666']);
			}
		},

		//首次数据渲染
		loadStatData: function (param) {
			$.post("/auth/provost/teachingMonitor/attendance/queryAttendancePageData.htm", param, function (json) {
				var data = json.datas.resultBean;
				var countBean = data.countBean;
				var trendList = data.trendList;
				var compList = data.compareBeanList;
				if (data) {
					var pieData = RenderEchart.buildStatPieData(5, ["全勤", "迟到", "早退", "迟到且早退", "缺勤"], [countBean.allOn, countBean.late, countBean.early, countBean.lateAndEarly, countBean.notClassNum]);
					RenderEchart.renderStatInfoPie("statInfo", pieData, ['#1fb5ab', '#ff9900', '#619eed', '#ffcb6b', '#ff6666']);
					//走势
		...
		loadDtlDataPage: function (param) {
			var url = "/auth/provost/teachingMonitor/attendance/queryAttendanceDetailDataPage.htm?startDate=" + param.startDate + "&endDate=" + param.endDate +
		...
		loadTrendData: function (param) {
			$.post("/auth/provost/teachingMonitor/attendance/queryAttendanceTrendData.htm", param, function (json) {
				var data = json.datas.resultBean;
				var trendList = data.trendList;
				if (data) {
					//处理日期趋势数据
		...
		loadCompData: function (param) {
			$.post("/auth/provost/teachingMonitor/attendance/queryAttendanceCompareData.htm", param, function (json) {
				var data = json.datas.resultBean;
				var compList = data.compareBeanList;
				if (data) {
					//处理横向对比数据
