...
<div class="g-bd">
	<div class="g-mn">
		...
		<div class="m-search-box">
			<div class="c-typeswitch">
				<ul id="ulSwitch15">
					...
					<li class="c-typeswitch__item active"><a href="/auth/provost/studentMonitor/readyAnalyse/readyAnalysePage.htm">预习</a></li>
		...
		<div class="c-analyse__con" id="statData">
			...
			<div class="c-zoom">
				<div class="title">全面预习率排行</div>
				<div class="c-anarank" style="width:100%">
					<div class="c-anarank__sort">
						<div class="c-anarank__title c-anarank__title--top">全面预习率前5名</div>
						<ul class="c-anarank__list" id="topFiveBody">
						</ul>
					</div>
					<div class="c-anarank__sort">
						<div class="c-anarank__title c-anarank__title--last">全面预习率后5名</div>
			...
			<div class="m-table m-table-center m-table-fixed">
				<table>
					...
					<tr>
						...
						<th width="8%">班级</th>
						<th width="6%">姓名</th>
						<th width="10%" id="readyLessonNum" class="m-sorting">应预习课堂数<i></i></th>
						<th width="12%" id="allReady" class="m-sorting m-sorting-asc">全面预习率（数）<i></i></th>
						<th width="12%" id="partReady" class="m-sorting">部分预习率（数）<i></i></th>
						<th width="10%" id="noReady" class="m-sorting">未预习率（数）<i></i></th>
...
{{#countBean}}
    <p class="disc">
		共学生<span class="s-green">{{studentTotalNum}}</span>人，需预习课堂<span class="s-green">{{lessonTotalNum}}</span>堂
	</p>
	<p>
		<i style="background-color:#619eed"></i>全面预习<span class="s-green">{{allReady}}</span>人堂，占比<span class="s-green">
		{{#cif 'this.allReadyPro == 100'}}
			100%
		{{else}}
			{{allReadyPro}}%
		{{/cif}}
	...
	<p>
		<i style="background-color:#ffcb6b"></i>部分预习<span class="s-green">{{partReady}}</span>人堂，占比<span class="s-green">
		{{#cif 'this.partReadyPro == 100'}}
			100%
		{{else}}
			{{partReadyPro}}%
		{{/cif}}
	...
	<p>
		<i style="background-color:#ff6666"></i>未预习<span class="s-green">{{noReady}}</span>人堂，占比<span class="s-green">
		{{#cif 'this.noReadyPro == 100'}}
			100%
		{{else}}
			{{noReadyPro}}%
		{{/cif}}
...
{{#dataList}}
    <tr>
    	...
    	<td>{{className}}</td>
    	<td>{{studentName}}</td>
    	<td>{{readyLessonNum}}</td>
    	<td>{{allReadyPro}}%（{{allReady}}）</td>
    	<td>{{partReadyPro}}%（{{partReady}}）</td>
    	<td>{{noReadyPro}}%（{{noReady}}）</td>
    	<td class="operation">
...
<script type="text/javascript">
	seajs.use('diag/studentMonitor/readyAnalysePage');
</script>
</html>