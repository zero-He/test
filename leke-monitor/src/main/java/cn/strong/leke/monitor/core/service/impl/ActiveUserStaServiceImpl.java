...
public class ActiveUserStaServiceImpl implements IActiveUserStaService{
	...
	public List<SchoolActiveUserStat> getDistrictSchoolActiveUser(ActiveUserQuery query) {
		...
		// 学校ID
		List<Long> sellerSchoolIds = ListUtils.map(querySellerSchool, e ->
		{
			return e.getSchoolId();
		});
		
		List<SchoolActiveUserStat> schoolActiveUserStats = aggregateUserInfo(query, sellerSchoolIds);
	...
	public  List<SchoolActiveUserStat> aggregateUserInfo(ActiveUserQuery query,List<Long> sellerSchools){
		...
		if (sellerSchools.size() != 0) {
			...
			case "日":
				...
				for (Integer date : staDay) { 
					...
					for (SchoolActiveUserStat schoolActiveUserStat : activeUser) {
						int countUsersByRole = userActiveStatisDao.countUsersByRole(schoolActiveUserStat.getSchoolId(), query.getRoleId(),query.getEndDate());
						schoolActiveUserStat = setUserInfo(schoolActiveUserStat);
			...
			case "周":
				...
				for (Integer week : staWeek) {
					...
					List<SchoolActiveUserStat> activeUser = dayActiveUserDao.getActiveUser(query, sellerSchools);
					for (SchoolActiveUserStat schoolActiveUserStat : activeUser) {
						int countUsersByRole = userActiveStatisDao.countUsersByRole(schoolActiveUserStat.getSchoolId(), query.getRoleId(),endDayOfWeek);
						schoolActiveUserStat = setUserInfo(schoolActiveUserStat);
			...
			case "月":
				...
				for (Integer month : staMonth) {
					...
					List<SchoolActiveUserStat> activeUser = dayActiveUserDao.getActiveUser(query, sellerSchools);
					for (SchoolActiveUserStat schoolActiveUserStat : activeUser) {
						int countUsersByRole = userActiveStatisDao.countUsersByRole(schoolActiveUserStat.getSchoolId(), query.getRoleId(),lastDayOfMonth);
						schoolActiveUserStat = setUserInfo(schoolActiveUserStat);
