 ...
 */
@Service
public class ExceptionRecordService implements IExceptionRecordService {
	...
	private MongoCollection<ExceptionRecord> coll;

	@PostConstruct
	public void init() {
		Assert.notNull(db, "DB should not be null");
		coll = db.getCollection("exception.record", ExceptionRecord.class);
	}

	@Override
	public void add(ExceptionRecord record) {
		record.setId(new ObjectId().toString());
		coll.insertOne(record);
	}

	@Override
	public List<ExceptionRecord> findExceptionRecords(ExceptionRecordQuery query, Page page) {
	...
	@Override
	public List<ExceptionStat> findExceptionStats(ExceptionRecordQuery query) {
		if (query == null) {
			throw new IllegalArgumentException("query should not be null.");
		}
		Document filter = query.toBSON();
		Bson match = new Document("$match", filter);
		return coll.aggregate(Arrays.asList(match, STAT_GROUP, STAT_PROJECT), ExceptionStat.class)
				.into(new ArrayList<>());
	}
